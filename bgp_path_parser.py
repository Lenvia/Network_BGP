...
class BgpPaths(object):
    ...
    def parse_bgp_paths(self, rib_file):
        ...
        with open(rib_file) as f:
            for line in f:
                ...
                # remove IXPs（互联网交换中心，一般由第三方中立运营）
                for asn in asn_list:
                    if asn in self.ixp:
                        asn_list.remove(asn)
                # remove prepended ASes（前置AS？）
    ...
    def output_forward_paths(self):
        f = open('./middle/'+mode+'_'+year+'_sanitized_rib.txt', 'w')
...
if __name__ == '__main__':
    ...
    peeringdb = './resources/peeringdb_2_dump_'+year+'_01_01.json'

    path = BgpPaths()
    # path.extract_ixp(sys.argv[1])
    path.extract_ixp(peeringdb)
    path.parse_bgp_paths('./middle/'+mode+'_'+year+'_rib.txt')
